...
def update_visibility(sprites, visibility_radius, custom_sprites):
    ...
    for sprite in sprites:
        if not is_point_behind_viewpoint(sprite.pos_3d, viewpoint=(0, 0, 0), view_direction=(0, 0, 1)):
...
def generate_frame_sprites(control_vars,canvas):
    ...
    frame_surfaces = create_circle_surfaces(CIndex.CYAN, 4 ) 
    frame_surfaces1 = create_circle_surfaces(CIndex.RED, 4)
    
    frame_surfaces2 = create_circle_surfaces(CIndex.GREEN3, 2) 
...
def handle_events(control_vars, key_to_scale, parsecs, current_orientation,rotate,x_offset,y_offset,canvas_scale,parsecs_changed):
    ...
    return True, parsecs, current_orientation, rotate,scaled_mouse_x, scaled_mouse_y,clearInfo, parsecs_changed
...
def main():
 ...
 #   star_data_np = load_custom_star_data('../datasets/star_database_colors.json')
    ...
    while running:
        ...
        # check for key input
        running, parsecs, current_orientation,rotated,mouse_x,mouse_y,clearInfo, parsecs_changed = handle_events(control_vars, key_to_scale, parsecs, current_orientation,rotated,x_offset,y_offset,canvas_scale,parsecs_changed)
        if running:
            canvas.fill((0, 0, 0))
            ...
            if parsecs_changed:
                #custom_visible_star_sprites = update_visibility(all_stars_sprites_group, control_vars['sphere'] * ScreenScaler,custom_visible_star_sprites)
                parsecs_changed = False
                rotated = True

            # Step 2: Update only the visible stars using the map
            if rotated :
                for star in all_stars_sprites_group:
