...
# draws the sun at the centre of the image
    ...
    screen.blit(twinkle_star(sun[10]), (canvas_width/2-offset, canvas_height/2-offset), special_flags=pygame.BLEND_ADD)
    screen.blit(star_labels['sol'], (canvas_width//2 - star_labels['sol'].get_width()//2, canvas_height/2 + star_labels['sol'].get_height()//2))
...
def load_custom_star_data(json_file_path):
    ...
    # missing_hr_ids.to_csv('../datasets/missing_stars.txt', index=False, header=False)
        ...
        for row in array_copy:
            startypeIndex = find_closest_star_index((row[SIndex.COLOR_K_R] *255, row[SIndex.COLOR_K_G]*255, row[SIndex.COLOR_K_B]*255))
...
def main():
 ...
 #                   if control_vars['draw_frame'] == True:
 #                       control_vars['draw_frame'] = False
 #                       print("Frame Off")
 #                   else:
 #                       control_vars['draw_frame'] = True
 #                       print("Frame On")
        ...
        if control_vars['draw_frame']:
            ...
            for row in updated_circle:
                ...
                rotated_vector = current_orientation.rotate([row[0],row[1],row[2]])
                nx, ny, nz = rotated_vector[0], rotated_vector[1], rotated_vector[2]
                row[0] = nx
                row[1] = ny
                row[2] = nz
            ...
            for row in updated_spokes:
                ...
                if spoke == 1:
                    ...
                    GCz = nz
                spoke +=1
            canvas.blit(galactic_label, (GCx+(canvas_width/2) - galactic_label.get_width()//2, GCy+(canvas_height/2) + galactic_label.get_height()//2))
        ...
        rotated_vector = current_orientation.rotate([galactic_center_star[SIndex.Dx], galactic_center_star[SIndex.Dy], galactic_center_star[SIndex.Dz]])
        galactic_center_coords = (rotated_vector[0], rotated_vector[1], rotated_vector[2])
        ...
        for star in star_data_np:
            if star[SIndex.DISTANCE_PARSECS] > control_vars['sphere']:
                ...
                rotated_vector = current_orientation.rotate([star[SIndex.Dx], star[SIndex.Dy], star[SIndex.Dz]])
                nx, ny, nz = rotated_vector[0], rotated_vector[1], rotated_vector[2]
 ...
 #               else:
                ...
                #starimagewidth = canvas_set([int(star[SIndex.STAR_TYPE])][magindex]).get_width()//2
                    canvas.blit(twinkle_star(canvas_set[int(star[SIndex.STAR_TYPE])][magindex]), (nx*scale+(canvas_width/2-starimagewidth), ny*scale+(canvas_height/2)-starimagewidth), special_flags=pygame.BLEND_ADD)

#                canvas.blit(twinkle_star(canvas_set[find_closest_star_index(color)][mag]), (nx*scale+(canvas_width/2), ny*scale+(canvas_height/2)), special_flags=pygame.BLEND_ADD)
...
#        galactic_center_coords = (rotated_vector[0], rotated_vector[1], rotated_vector[2])
        ...
        # Draw the line towards the galactic center
#        start_point = (canvas_width // 2, canvas_height // 2)
#        end_point = (int(galactic_center_coords[0] * scale + canvas_width // 2), int(galactic_center_coords[1] * scale + canvas_height // 2))
#        pygame.draw.line(canvas, CIndex.RED, start_point, end_point, 5)
