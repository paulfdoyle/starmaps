...
DEFAULT_SYSTEM_MESSAGE = ('You are a scientist at a planetarium, who excels at educating the general '
                          ...
                          'the distance from Earth, and one interesting fact which the public might find interesting but dont call it an interesting fact, just give the information. '
...
# Your existing LLMAPIClient class (unchanged)
@dataclass
...
json_file_path = '../datasets/merged_star_exo_data.json'  # Path to your JSON file
...
for star in stars_data:
    ...
    star_info = f"index: {index}, Star: {proper_name}, HIP ID: {hip_id}, HD Reference: {hd_ref}"
    ...
    # Add the response to the star data under the "info" field
    star['info'] = response

# Write the updated data back to the JSON file
updated_json_file_path = '../datasets/updated_merged_star_exo_data.json'
with open(updated_json_file_path, 'w') as file:
    json.dump(stars_data, file, indent=4)

print(f"Updated JSON data has been written to {updated_json_file_path}")
