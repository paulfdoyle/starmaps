...
def pregenerate_frame_points(radius, num_circles=4, num_spokes=8):
    radius = radius * 0.8
    circle_points = []
...
#                     control_vars['sphere'] = control_vars['sphere'] + (control_vars['sphere'] *0.1)
#                     max = 0
...
#         end_point = (int(nx * scale + canvas_width // 2), int(ny * scale + canvas_height // 2))
#         # Debug output for line coordinates
...
#                 #color = (int(star[SIndex.COLOR_K_R] * 255), int(star[SIndex.COLOR_K_G] * 255), int(star[SIndex.COLOR_K_B] * 255))
#                 canvas.blit(twinkle_star(canvas_set[int(star[SIndex.STAR_TYPE])][mag]), (nx*scale+(canvas_width/2), ny*scale+(canvas_height/2)), special_flags=pygame.BLEND_ADD)
...
def main():
    ...
    while running:
        ...
        angle_x += angle_x_rate
        angle_y += angle_y_rate
